...
response is **'application/json** with **MediaType.APPLICATION_JSON'** instead of the **'application/json** with **MediaType.APPLICATION_JSON'** is better because if there's a spelling error, a compile failure occurs.
